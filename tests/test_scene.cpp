...
static void t_showcase() {
    test_current = "showcase";
    scene_data show = build_scene("showcase", 1200.0 / 675.0, 0.0);
    EXPECT_TRUE((int)show.lights.size() == 3); // overhead key + warm orb + neon rim
    EXPECT_TRUE((int)show.media.size() == 2);  // SSS + smoke
    EXPECT_TRUE((int)show.objs.size() >= 20);
    hittable_list world;
    for (auto &o : show.objs)
        world.add(o);
    hit_record hr;
    EXPECT_TRUE(world.hit(show.cam.get_ray(0.5, 0.5), 0.001, 1e30, hr));
    flat_scene fs;
    EXPECT_TRUE(flatten_scene(show, fs));
    EXPECT_TRUE(fs.nlights == 3);
    EXPECT_TRUE(!fs.gs.quads.empty());
    EXPECT_TRUE(!fs.gs.spheres.empty());
    EXPECT_TRUE(!fs.gs.tris.empty());
}
...
static void t_flatten() {
    ...
    // Posed box bakes to 6 quads flat.
    scene_data pb_scene;
    pb_scene.objs.push_back(make_posed_box(vec3(1, 1, 1), 15.0, vec3(0, 0, 0), ramp_mat));
    flat_scene fpb;
    EXPECT_TRUE(flatten_scene(pb_scene, fpb));
    EXPECT_TRUE((int)fpb.refs.size() == 6);
...
static void t_expand_gpu() {
    ...
    // showcase full scene flattens (instances + qbvh + motion + media).
    scene_data show = build_scene("showcase", 1.0, 1.0);
    flat_scene fshow;
    EXPECT_TRUE(flatten_scene(show, fshow));
    EXPECT_TRUE(fshow.nlights == 3);
}
