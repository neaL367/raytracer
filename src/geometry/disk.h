...
public:
    disk() : radius(1.0), inner_radius(0.0) { init_static(); }
    ...
    void init_static() {
        ...
        inv_r_span = (radius > inner_radius) ? (1.0 / (radius - inner_radius)) : 0.0;
    }
    ...
    bool hit(const ray &r, double t_min, double t_max, hit_record &rec) const override {
        ...
        double t = (D - dot(r.origin(), normal)) / denom;
        if (t < t_min || t > t_max)
            return false;
        ...
        rec.v = (dist - inner_radius) * inv_r_span;
        rec.tangent = u_axis;
    ...
    bool hit_any(const ray &r, double t_min, double t_max) const override {
        ...
        return (dist2 <= r2_outer && dist2 >= r2_inner);
    }
