...
// Upload 4 data blobs as UBO + 3 SSBOs, wire descriptor set.
// bufs[0] = camera UBO, [1..3] = scene SSBOs (empty blobs get 16B pad;
// Vulkan forbids zero-size buffers).
inline void gpu_set_scene(GpuContext &g, const void *data[4], const size_t bytes[4]) {
    for (int i = 0; i < 4; ++i) {
